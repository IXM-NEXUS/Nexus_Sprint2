...
import getAccountDetails from '@salesforce/apex/QuoteController.getAccountDetails';
...
export default class QuoteModalAdvanced extends NavigationMixin(LightningElement) {
      ...
      @track totalPrice = 0;
                 ...
                 async initializeDefaultValues() {
                     try {
                         ...
                         this.quoteData = {
                             ...
                             endCustomer:opp.End_Customer__c?opp.End_Customer__r.Name:'',
                             endCustomerId:opp.End_Customer__c,
                         ...
                         this.selectedRecordId=opp.AccountId;
                         this.endCustomerId=opp.End_Customer__c;
                 ...
                 async saveQuoteAndContinue() {
                     try {
                         if (!this.quoteData.Pricebook2Id) {
                             ...
                             let quoteId;
                             console.log('selected account data ==>'+JSON.stringify(this.quoteData));
                             if (this.createdQuoteId) {
                                console.log('update quote Data ==>'+JSON.stringify(this.quoteData));
                                ...
                                console.log('create new quoteData==>', JSON.stringify(this.quoteData));
                ...
                if (hasDiscountedItems || isNotProfitable) {
                 ...
                 saveFormValuesToQuoteData() {
                     const form = this.template.querySelector('lightning-record-edit-form');
                        if (form) {
                            // Handle lightning-input-field components
                            ...
                            // Handle lightning-record-picker separately
                            const recordPicker = this.template.querySelector('lightning-record-picker');
                            if (recordPicker && this.selectedRecordId) {
                                this.quoteData.AccountId = this.selectedRecordId;
    ...
    setTimeout(() => {
        ...
        this.showNotification('Success', 'Quote line item cloned successfully.', 'success');
                ...
                async handleRecordSelection(event) {
                    console.log('handleRecordSelection called with recordId: ' + JSON.stringify(event.detail));
                    this.selectedRecordId=event.detail.recordId;
                    this.quoteData.AccountId = this.selectedRecordId;
                    
                     const acc = await getAccountDetails({ accountId: this.selectedRecordId });
                     this.quoteData.accountName=acc.Name;
                    this.quoteData.BillingCountry=acc.BillingCountry;
                    this.quoteData.ShippingCountry=acc.ShippingCountry;
                    this.quoteData.BillingStreet =acc.BillingStreet;
                    this.quoteData.BillingCity=acc.BillingCity;
                    this.quoteData.BillingPostalCode=acc.BillingPostalCode;
                    this.quoteData.BillingState=acc.BillingState;
                    this.quoteData.ShippingState=acc.ShippingState;
                    this.quoteData.ShippingStreet=acc.ShippingStreet;
                    this.quoteData.ShippingCity=acc.ShippingCity;
                    this.quoteData.ShippingPostalCode=acc.ShippingPostalCode;
                    this.quoteData.ShippingState= acc.ShippingState;
                    this.quoteData.ShippingPostalCode=acc.ShippingPostalCode;
                    this.quoteData.BillingName= acc.Name;
                    this.quoteData.ShippingName= acc.Name;
